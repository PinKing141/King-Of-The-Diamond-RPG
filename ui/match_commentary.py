...
from match_engine.states import EventType, HitType
from ui.ui_display import (
    Colour,
    render_battery_call_banner,
    render_battery_forced_banner,
    render_battery_shake_banner,
...
def _emit(message: str, *, level: str = "info", io: Optional[IOInterface] = None) -> None:
    if io:
        io.log(message, level=level)
        return
    print(_clean_text(message))
...
def _format_error_entries(entries: Optional[List[Any]]) -> str:
    if not entries:
        return "None"
    labels: List[str] = []
...
class CommentaryListener:
    ...
    def _log(self, message: str, *, level: str = "info") -> None:
        _emit(message, level=level, io=self.io)
    ...
    def _on_minigame_trigger(self, payload: Dict[str, Any]) -> None:
        ...
        if self.io:
            render_clutch_banner(
                ...
                count=context.get("count", "3-2"),
                score_diff=context.get("score_diff", 0),
                runners_on=context.get("runners_on", 0),
                label=context.get("label", "Clutch Pitch"),
            )

    def _on_minigame_resolve(self, payload: Dict[str, Any]) -> None:
        if not commentary_enabled():
            return
    ...
    def _on_battery_sign(self, payload: Dict[str, Any]) -> None:
        if not commentary_enabled():
            return
        pitcher = self._player_label(payload.get("pitcher_id"), "Pitcher")
        catcher = self._player_label(payload.get("catcher_id"), "Catcher")
        if self.io:
            render_battery_call_banner(payload, pitcher_name=pitcher, catcher_name=catcher)
        else:
            self._log(
                f"   >> Catcher Sign: {pitcher}/{catcher} {payload.get('pitch_name', '')} ({payload.get('intent', 'Plan')}) | Trust {payload.get('trust', '??')} Sync {payload.get('sync', 0):+.2f}"
            )

    def _on_battery_shake(self, payload: Dict[str, Any]) -> None:
        if not commentary_enabled():
            return
        pitcher = self._player_label(payload.get("pitcher_id"), "Pitcher")
        catcher = self._player_label(payload.get("catcher_id"), "Catcher")
        if self.io:
            render_battery_shake_banner(payload, pitcher_name=pitcher, catcher_name=catcher)
        else:
            shakes_used = payload.get("shakes_used")
            allowed = payload.get("shakes_allowed")
            self._log(
                f"   >> Shake-Off {shakes_used}/{allowed}: {pitcher} waves off {catcher}"
            )

    def _on_battery_forced(self, payload: Dict[str, Any]) -> None:
        if not commentary_enabled():
            return
        pitcher = self._player_label(payload.get("pitcher_id"), "Pitcher")
        catcher = self._player_label(payload.get("catcher_id"), "Catcher")
        if self.io:
            render_battery_forced_banner(payload, pitcher_name=pitcher, catcher_name=catcher)
        else:
            sync = payload.get("sync")
            self._log(
                f"   >> Forced Call: {catcher} overrides {pitcher} (sync {sync:+.2f})"
            )

    def _on_pickoff_event(self, payload: Dict[str, Any]) -> None:
        if not commentary_enabled():
            return
...
def _announce_weather_once(state) -> None:
    ...
    _emit(f"   >> {icon} Weather watch: {summary}.{extra}")
...
def display_state(state, pitcher, batter):
    ...
    _emit(f" {away_name[:3]} {state.away_score}  -  {state.home_score} {home_name[:3]}   |   {state.top_bottom} {state.inning}")
    _emit("=" * 60)
    ...
    _emit(f" BATTER:  {b_name} {b_cond} (Pow {batter.power} / Con {batter.contact})")
    _emit(f" COUNT:   {state.balls}-{state.strikes}  |  OUTS: {state.outs}")
    _emit("-" * 60)
...
def _render_batters_eye_panel(state):
    ...
    for entry in history:
        ...
        _emit(f"   {icon} [{tag}] {label} at {count}")
    _emit("-" * 60)
...
def announce_pitch(pitch_result):
    ...
    if pitch_result.outcome == "Ball":
        _emit(f" > {pitch_result.pitch_name} ({vel_str}) ... Ball {pitch_result.description}.")
        ...
        _emit(f" > {pitch_result.pitch_name} ({vel_str}) ... STRIKE! ({_clean_text(desc)})")
    elif pitch_result.outcome == "Foul":
        _emit(f" > {pitch_result.pitch_name} ... Fouled off.")
    elif pitch_result.outcome == "InPlay":
        _emit(f" > {pitch_result.pitch_name} ... CONTACT!")
...
def announce_play(contact_result):
    ...
    if getattr(contact_result, 'error_on_play', False):
        ...
        _emit(f"   >> [{label}] {defender}: {contact_result.description}")
        return
    if hit_type == HitType.OUT:
        _emit(f"   >> {contact_result.description}")
        if "Strikeout" in contact_result.description:
             _emit(f"   >> {rng.choice(STRIKEOUT_PHRASES)}")
    elif hit_type == HitType.HOMERUN:
        _emit(r"       _   _  ____  __  __  ___   ____  _   _  _   _ ")
        _emit(r"      | | | |/ __ \|  \/  || __| |  _ \| | | || \ | |")
        _emit(r"      | |_| | |  | | \  / || _|  | |_) | | | ||  \| |")
        _emit(r"      |  _  | |__| | |\/| || |__ |  _ <| |_| || |\  |")
        _emit(r"      |_| |_|\____/|_|  |_||___| |_| \_\\___/ |_| \_|")
        _emit(f"   >> ** {rng.choice(HOMERUN_PHRASES)} **")
    elif hit_type in {HitType.SINGLE, HitType.DOUBLE, HitType.TRIPLE}:
        base = hit_type.value
        _emit(f"   >> {base}! {rng.choice(HIT_PHRASES)}")
    else:
        _emit(f"   >> {contact_result.description}")
...
def announce_score_change(runs, batting_team_name):
    if not _COMMENTARY_ENABLED:
        return
    if runs > 0:
        _emit(f"   !! {runs} RUN(S) SCORED for {batting_team_name}! !!")
...
def game_over(state, winner):
    ...
    _emit(f" Final Score: {away_name} {state.away_score} - {state.home_score} {home_name}")
    _emit(f" Winner: {winner_name}")
    ...
    if umpire:
        ...
        _emit(f" Plate Umpire: {umpire.name} — {desc}")
        _emit(f"   Zone bias {zone:+.2f} | Home lean {home_bias:+.2f}")
        _emit(
            f"   Strictness {getattr(umpire, 'strictness', 0.5):+.2f} | Consistency {getattr(umpire, 'consistency', 0.7):+.2f} | Framing {getattr(umpire, 'framing_factor', 0.0):+.2f}"
        )
        _emit(f"   Calls: Home +{home_tilt.get('favored', 0)} / -{home_tilt.get('squeezed', 0)} | Away +{away_tilt.get('favored', 0)} / -{away_tilt.get('squeezed', 0)}")
    summary = getattr(state, 'error_summary', None)
    normalized = _normalize_error_summary(summary)
    away_tags = _format_error_entries(normalized.get('away'))
    home_tags = _format_error_entries(normalized.get('home'))
    if away_tags != "None" or home_tags != "None":
        _emit(f" Errors: {away_name[:3]} {away_tags} | {home_name[:3]} {home_tags}")
    _emit("#" * 60 + "\n")


__all__ = [
    "CommentaryListener",
    "set_commentary_enabled",
    "commentary_enabled",
    "display_state",
    "announce_pitch",
    "announce_play",
    ...
    "game_over",
    "get_recent_batters_eye_history",
    "type_writer",
    "attach_commentary_listener",
]


def attach_commentary_listener(bus, *, io: Optional[IOInterface] = None):
    """Attach CommentaryListener to a bus and keep a reference to avoid GC."""
    if bus is None:
        return None
    listener = CommentaryListener(bus, io=io)
    setattr(bus, "_commentary_listener", listener)
    return listener
